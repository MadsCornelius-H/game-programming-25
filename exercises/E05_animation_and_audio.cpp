...
#define NumWalkFrames 8
#define Sprite_Change_Frame 10
...
struct PlayerData
{
	...
	float g; // gravity
	SpriteTexture *walkSpriteTextures;
	SpriteTexture *jumpFallSpriteTextures;
...
static void Load_textures_and_assign(GameState *state)
{

	state->player_data.walkSpriteTextures = (SpriteTexture *)SDL_calloc(NumWalkFrames, sizeof(SpriteTexture));
	state->player_data.jumpFallSpriteTextures = (SpriteTexture *)SDL_calloc(2, sizeof(SpriteTexture));

	tinyxml2::XMLDocument doc;
	if (doc.LoadFile("data/kenney/character_femalePerson_sheet.xml") != tinyxml2::XML_SUCCESS)
	{
		 ...
		 elem = elem->NextSiblingElement("SubTexture"))
	{
		...
		if (std::strncmp(name, "walk", 4) == 0)
		{
			SpriteTexture *tex = &state->player_data.walkSpriteTextures[index];
			tex->x = elem->FloatAttribute("x");
			tex->y = elem->FloatAttribute("y");
			tex->width = elem->FloatAttribute("width");
			tex->height = elem->FloatAttribute("height");
			index++;
		}
		else if (std::strncmp(name, "jump", 4) == 0)
		{
			SpriteTexture *tex = &state->player_data.jumpFallSpriteTextures[0];
			tex->x = elem->FloatAttribute("x");
			tex->y = elem->FloatAttribute("y");
			tex->width = elem->FloatAttribute("width");
			tex->height = elem->FloatAttribute("height");
		}
		else if (std::strncmp(name, "fall", 4) == 0)
		{
			SpriteTexture *tex = &state->player_data.jumpFallSpriteTextures[1];
			tex->x = elem->FloatAttribute("x");
			tex->y = elem->FloatAttribute("y");
			tex->width = elem->FloatAttribute("width");
			tex->height = elem->FloatAttribute("height");
...
static void game_init(SDLContext *context, GameState *state)
{
	...
	// Read xml and extract the sprite walk animations
...
static void game_reset(SDLContext *context, GameState *state)
{
	...
	// player
	{
		...
		Load_textures_and_assign(state);
		state->player_data.frame_change = Sprite_Change_Frame;
		state->player_data.texture_counter = 0;
...
static void game_update(SDLContext *context, GameState *state)
{
	// player
	{
		...
		{
			idle = false;
			if (state->player_data.frame_change <= 0)
			{
				state->player_data.frame_change = 10;
				state->player_data.texture_counter += 1;
				if (state->player_data.texture_counter >= 8)
				{
					state->player_data.texture_counter = 0;
				}
				SpriteTexture text = state->player_data.walkSpriteTextures[state->player_data.texture_counter];
				state->player->sprite.rect = SDL_FRect{text.x, text.y, text.width, text.height};
			}

			if (abs(entity->physics_data.velocity.x) > 6.0f)
			{
				state->player_data.frame_change -= 2;
			}
			else
			{
				state->player_data.frame_change -= 1;
			}
		}
		if (velocity.y > 0)
		{
			// we are jumping
			idle = false;
			SpriteTexture text = state->player_data.jumpFallSpriteTextures[0];
			state->player->sprite.rect = SDL_FRect{text.x, text.y, text.width, text.height};
		}
		else if (velocity.y < 0 && (!data->is_grounded))
		{
			// we are falling
			idle = false;
			SpriteTexture text = state->player_data.jumpFallSpriteTextures[1];
			state->player->sprite.rect = SDL_FRect{text.x, text.y, text.width, text.height};
		}

		if (idle)
		{
			state->player->sprite.rect = SDL_FRect{0, 0, 98, 128};
		}
	...
	// door
	{
		...
		float dist = distance(state->player->transform.position, vec2f{7.0f, 1.5f});
		...
		// advance animation
		door->animTime += context->delta; // dt = frame delta time
		
		if (door->animTime > door->animDuration)
			door->animTime = door->animDuration;

		// normalized t in [0,1]
		float t = door->animTime / door->animDuration;

		vec2f Start = door->isOpening ? vec2f{0.0f,1.5f} :vec2f{0.0f,4.0f};
		vec2f End = door->isOpening ? vec2f{0.0f,4.0f} :vec2f{0.0f,1.5f};
		...
		else
		{
			...
			new_position = newEasingPosition(Start,End,eased);


		// directly set transform (skip linear velocity)
		float currentY = door->transform.position.y;

		float velocityY = (new_position.y - currentY) / context->delta;
		// also sync physics body
		if (context->delta > 0.0f) {
    		float velocityY = (new_position.y - currentY) / context->delta;
    		velocityY = std::clamp(velocityY, -10.0f, 10.0f); // prevent NaN/Inf
    		b2Body_SetLinearVelocity(door->physics_data.body_id, b2Vec2{0.0f, velocityY});
}
